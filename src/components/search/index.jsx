...
import { resultChange, resultSelect } from '../../actions/search';

const last = (array) =>
	(array.length) > 0 ? array[array.length - 1] : null;

function Search({ lastQuery, lastResult, onResultChange, onResultSelect }) {
	return (
		<FacetedSearch
			config={{
				baseURL: baseUrl,
...
export default connect(
	state => ({
		lastQuery: last(state.search.queries),
		lastResult: last(state.search.results),
	}),
	{
